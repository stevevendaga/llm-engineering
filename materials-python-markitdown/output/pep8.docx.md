...
underscores) such as \_\_all\_\_, \_\_author\_\_, \_\_version\_\_,
etc. should be placed after the module docstring but before any import
statements *except* from \_\_future\_\_ imports. Python mandates that
...
from **future** import barry\_as\_FLUFL
...
tuple whose items traditionally have names like st\_mode,
st\_size, st\_mtime and so on. (This is done to emphasize the
...
* \_single\_leading\_underscore: weak "internal use" indicator.
  ...
  with an underscore.
* single\_trailing\_underscore\_: used by convention to avoid
  conflicts with Python keyword, e.g. :
* .. code-block::
   :class: good
* tkinter.Toplevel(master, class\_='ClassName')
* \_\_double\_leading\_underscore: when naming a class attribute,
  invokes name mangling (inside class FooBar, \_\_boo becomes
  \_FooBar\_\_boo; see below).
* \_\_double\_leading\_and\_trailing\_underscore\_\_: "magic" objects or
  attributes that live in user-controlled namespaces.
  E.g. \_\_init\_\_, \_\_import\_\_ or \_\_file\_\_. Never invent
...
the ``\_\_all\_\_`` mechanism to prevent exporting globals, or use the
...
use an abbreviation or spelling corruption. Thus ``class\_`` is better
...
attribute named \_\_a, it cannot be accessed by Foo.\_\_a. (An
insistent user could still gain access by calling Foo.\_Foo\_\_a.)
...
* Note 2: Name mangling can make certain uses, such as debugging and
  \_\_getattr\_\_(), less convenient. However the name mangling
...
names in their public API using the \_\_all\_\_ attribute. Setting
\_\_all\_\_ to an empty list indicates that the module has no public API.

Even with \_\_all\_\_ set appropriately, internal interfaces (packages,
...
API, such as os.path or a package's \_\_init\_\_ module that exposes
...
* When implementing ordering operations with rich comparisons, it is
  best to implement all six operations (\_\_eq\_\_, \_\_ne\_\_,
  \_\_lt\_\_, \_\_le\_\_, \_\_gt\_\_, \_\_ge\_\_) rather than relying
  on other code to only exercise a particular comparison.
* To minimize the effort involved, the functools.total\_ordering()
...
* The latter example doesn't provide any information to indicate that
  the \_\_enter\_\_ and \_\_exit\_\_ methods are doing something other
